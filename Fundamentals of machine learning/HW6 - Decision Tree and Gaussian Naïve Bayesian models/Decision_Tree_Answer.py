    ...
    def Find_Best_Feature(self, df):
        ...
        # ============       Edit here      ==================
            ...
            if idx in self.Category_feature_idx:
                sep_data = np.unique(df[h].values)
                list = []
                for i, val in enumerate(sep_data):
                    mask = (df[h].values == val)
                    tmp = Y_data[mask]
                    list.append(len(tmp) / len(Y_data) * impurity_func(tmp, self.criterion))
                impurity = np.sum(list)
            # Numeric Feature Case
            else:
                split_value = Finding_split_point(df, h, self.criterion)
                mask = (df[h].values < split_value)
                sep1 = Y_data[mask]
                sep2 = Y_data[~mask]
                lenh = len(df[h].values)
                impurity = len(sep1) / lenh * impurity_func(sep1, self.criterion)
                impurity += len(sep2) / lenh * impurity_func(sep2, self.criterion)
        #=====================================================
