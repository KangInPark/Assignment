...
def convolution2d(x, kernel, stride):
    ...
    for i in range(conv_height):
        for j in range(conv_width):
            list.append(np.sum(x[i*stride:i*stride + kernel_size, j*stride:j*stride + kernel_size] * kernel))
    conv_out = np.array(list).reshape(conv_height, conv_width)
    # =========================================================================
    return conv_out
...
class ConvolutionLayer:
        ...
        for i in range(batch_size):
            for j in range(out_channel):
                for k in range(in_channel):
                    conv[i][j] += convolution2d(x[i][k], kernel[j][k], stride) + bias[j]
        # =========================================================================
        return conv
    ...
    def backward(self, d_prev):
        ...
        ** [HINT] **
        ...
        # db
                    tmp = convolution2d(np.ones_like(self.x[i][k]), d_prev[i][j], self.stride)
                    self.db[j] += tmp[0][0]
        # dx
                    kernel = np.flip(self.W[j][k])
                    hpad = kernel.shape[0] - 1
                    wpad = kernel.shape[1] - 1
                    spad = self.stride - 1
                    t = np.zeros((d_prev[i][j].shape[0] + 2 * hpad + spad * (d_prev[i][j].shape[0] - 1), d_prev[i][j].shape[1] + 2 * wpad + spad * (d_prev[i][j].shape[1] - 1)))
                    chkh = 0
                    chkw = 0
                    idxh = 0
                    for p in range(hpad, t.shape[0] - hpad):
                        if chkh != 0:
                            chkh -= 1
                        else:
                            idxw = 0
                            chkh = spad
                            chkw = 0
                            for q in range(wpad, t.shape[1] - wpad):
                                if chkw != 0:
                                    chkw -= 1
                                else:
                                    chkw = spad
                                    t[p][q] = d_prev[i][j][idxh][idxw]
                                    idxw += 1
                            idxh += 1
                    dx[i][k] += convolution2d(t, kernel, 1)
        if self.pad > 0:
    ...
    def zero_pad(self, x, pad):
        ...
        for i in range(batch_size):
            for j in range(in_channel):
                padded_x[i][j][pad:pad + height, pad:pad + width] = x[i][j]
        # =========================================================================
        return padded_x
...
class MaxPoolingLayer:
    ...
    def forward(self, x):
        ...
        ks = self.kernel_size
        st = self.stride
        pool_width = int((width - ks) / st + 1)
        pool_height = int((height - ks) / st + 1)
        list = []
        for p in range(batch_size):
            for q in range(channel):
                for i in range(pool_height):
                    for j in range(pool_width):
                        tmp = x[p][q][i * st:i * st + ks, j * st:j * st + ks]
                        arg = np.argmax(tmp)
                        self.mask[p][q][i*st + arg//ks][j*st + arg%ks] = 1
                        list.append(np.max(tmp))
        max_pool = np.array(list).reshape((batch_size, channel, pool_height, pool_width))
        # =========================================================================
    ...
    def backward(self, d_prev=1):
        ...
        for i in range(batch):
            for j in range(channel):
                for p in range(d_prev[i][j].shape[0]):
                    for q in range(d_prev[i][j].shape[1]):
                        tmp = self.mask[i][j][p * st:p * st + ks, q * st:q * st + ks] * d_prev[i][j][p][q]
                        d_max[i][j][p * st:p * st + ks, q * st:q * st + ks] += tmp
        # =========================================================================
        return d_max
...
class FCLayer:
    ...
    def backward(self, d_prev):
        ...
        self.db = np.ones_like(d_prev.T).dot(d_prev)[0]
        dx = self.W.dot(d_prev.T).T
        # =========================================================================
        return dx
...
class SoftmaxLayer:
    ...
    def backward(self, d_prev=1):
        ...
        dx = None
        # =============================== EDIT HERE ===============================
        dx = (self.y_hat - self.y) / self.y.shape[0] * d_prev
        # =========================================================================
        return dx
    ...
    def ce_loss(self, y_hat, y):
        ...
        self.y = y
        # =============================== EDIT HERE ===============================
        self.loss = - np.mean(np.sum(y * np.log(y_hat + eps), axis=1))
        # =========================================================================
        return self.loss
